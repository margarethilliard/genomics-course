...
Much of the text in this lab is derived from Roger Peng’s R Programming for Data Science, the chapters Iteration and Functions (https://r4ds.had.co.nz/functions.html) chapters in R for Data Science by Hadley Wickham and supplements from Thomas Girke’s Data Analysis in Genome Biology.
...
```
This loop takes the i variable and in each iteration of the loop gives it values 1, 2, 3, …, 15, executes the code within the curly braces, and then the loop exits.

The following three loops all have the same behavior.
```{r}
x <- c("lamb", "sheep", "cow", "dog")
for(i in 1:4) {
...
```{r}
f <- function() {
  cat("Climate change is real!\n")
}

f()
```
The last aspect of a basic function is the function arguments. These are the options that you can specify to the user that the user may explicitly set. For this basic function, we can add an argument that determines how many times “Climate change is real” is printed to the console.
...
Obviously, we could have just cut-and-pasted the cat("Climate change is real!\n") code three times to achieve the  same effect, but then we would not be programming, would we? Also, it would be un-neighborly of you to give your code to someone else and force them to cut-and-paste the code however many times the need to see “Hello, world!”.
...
```{r}
f <- function(num) {
         climate <- "Climate change is real!\n"
         for(i in seq_len(num)) {
                 cat(climate)
         }
         chars <- nchar(climate) * num
         chars
 }
...
At this point, we have written a function that
    ...
    a) has one formal argument named num with a default value of 1. The formal arguments are the arguments included in the function definition. The formals() function returns a list of all the formal arguments of a function

    b) prints the message “Hello, world!” to the console a number of times indicated by the argument num

    c) returns the number of characters printed to the console
