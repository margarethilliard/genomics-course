...
Confirmed_State_joined_long <- Confirmed_State_joined %>% 
              pivot_longer(-c(Province_State),
                            names_to = "Date", values_to = "Confirmed")
View(Confirmed_State_joined_long)
```
Barplot 
Add more informative x and y labels and a title to the above graph in Ex2.
```{r, fig.width = 3, fig.height = 3}

ggplot(data = Confirmed_State_joined_long, aes(x = Province_State, y = log2(Confirmed))) +
  geom_bar(stat = "identity", aes(color = Date)) +
  theme(axis.text.x = element_text(size = 6, angle = 90)) +
  labs(title = "Confirmed Cases by State",
       x = "State/Province", 
       y = "Log2 Number Confirmed Cases")
...
top_death <- time_series_long_joined %>%
...
ggplot(data = top_death, mapping = aes(x = Country_Region, y = Total_Death)) +
  geom_bar(stat = "identity") +
  labs(title = "Highest COVID-19 Death total by Country",
       x = "Country",
       y = "Total Death") 
...
US_time_series_confirmed_long <- US_time_series_confirmed %>% 
  unite(Key, Province_State, sep = ".", remove = FALSE)
